...
# Additions
measurePhysicalTemperature = 'temperature'
measurePhysicalHumidity = 'humidity'
...
def createRandomMetrics(hostId, timestamp, timeUnit, extended=False):
    records = list()
    ...
    # My extension
    extendedMeasures = ['measurePhysical'] # temperature 80.0, humidity 45
    if extended:
        for measure in extendedMeasures:
            temperature_value = 70 + 10.0 * random.random()
            humidity_value = 45 + 10.0 * random.random()
            value = '{"physical": {"temperature":' + str(temperature_value) + ', "humidity":'+ str(humidity_value) + '}}'
            records.append(createRecord(measure, value, "VARCHAR", timestamp, timeUnit))
